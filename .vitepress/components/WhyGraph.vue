<template>
  <div class="why-graph-wrap">
    <div class="wg-header">
      ...
      <span class="wg-title">{{ texts.title }}</span>
    </div>
    <div class="wg-body">
      <VueFlow
        :nodes="nodes"
...
const copy = {
  zh: { title: '云资源采购单 - 联动依赖图' },
  en: { title: 'Cloud procurement form - dependency graph' },
} as const
...
<style scoped>
.why-graph-wrap { background: var(--vp-c-bg-soft); border: 1px solid var(--vp-c-border); border-radius: 10px; overflow: hidden; margin: 20px 0; }
.wg-header { display: flex; align-items: center; gap: 10px; padding: 10px 16px; border-bottom: 1px solid var(--vp-c-border); }
.wg-badge { font-size: 10px; font-weight: 800; font-family: 'JetBrains Mono', monospace; padding: 2px 8px; border-radius: 4px; background: rgba(99,102,241,.15); color: #818cf8; border: 1px solid rgba(99,102,241,.3); letter-spacing: 0.06em; }
.wg-title { font-size: 12px; font-weight: 600; color: var(--vp-c-text-2); }
.wg-body { height: 260px; }
.wg-flow { width: 100%; height: 100%; background: transparent !important; }
...
.wg-node { display: flex; align-items: center; gap: 7px; padding: 6px 12px; border-radius: 7px; border: 1px solid; font-size: 11px; font-family: 'JetBrains Mono', monospace; font-weight: 600; white-space: nowrap; cursor: default; }
.wn-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
.wg-node.input { background: rgba(99,102,241,.1); border-color: rgba(99,102,241,.4); color: #818cf8; }
.wg-node.input .wn-dot { background: #6366f1; }
.wg-node.computed { background: rgba(245,158,11,.1); border-color: rgba(245,158,11,.4); color: #fbbf24; }
.wg-node.computed .wn-dot { background: #f59e0b; }
.wg-node.output { background: rgba(52,211,153,.1); border-color: rgba(52,211,153,.4); color: #34d399; }
